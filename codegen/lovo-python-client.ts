...
export function renderLovoPythonClient(contract: LovoContract): string {
  ...
  const compile = (raw: unknown, name: string, value: string, depth = 0): { type: string; check: string } => {
    ...
    if (schema.type === "object") {
      ...
      checks.push(`is_sequence(${value})`, `all(${part.check} for ${item} in ${value})`);
    } else {
    ...
    return schema.nullable === true ? { type: `${type} | None`, check: `(${value} is None or ${check})` } : { type, check };
  };
...
async def ${name}(value: ${inputName}, *, api_key: str, base_url: str, transport: HttpTransport) -> HttpResponse:
    ...
    return await transport.send(HttpRequest(${literal(op.method.toUpperCase())}, target,
        ...
        ${op.body ? 'json.dumps(value, allow_nan=False, separators=(",", ":")).encode("utf-8")' : 'b""'}))
`);
  }
