import { renderCambPythonClient } from "./camb-python-client.ts";
import { renderCambGoClient } from "./camb-go-client.ts";
import { renderCambRustClient } from "./camb-rust-client.ts";
...
export function renderCambClient(openapi: unknown, asyncapi: unknown, urls: readonly string[]) {
  ...
  const contract = {
    http: (value: unknown) => resolve(http, value), live: (value: unknown) => resolve(live, value), input: inputSchema,
    messages: definitions, groups, baseUrl, webSocketUrl: `${server.protocol}://${server.host}${server.pathname}`,
    path, method, header: scheme.name, urls,
  };
  return {
    languages: `// Generated by codegen/generate-clients.ts from ${urls[0]}. Do not edit.\nexport type Language = ${language.type};\n`,
    python: renderCambPythonClient(contract),
    go: renderCambGoClient(contract),
    rust: renderCambRustClient(contract),
