...
// Preserve the prevalent emitted ordering while making annotation order irrelevant.
const constraintOrder = ["integer", "minimum", "exclusiveMinimum", "maximum", "maxLength", "pattern", "minItems", "maxItems", "itemInteger", "itemMinimum", "itemMaximum"];
...
export function identity(type: SchemaType): string {
  // Rendered TypeScript is diagnostic text, never a source of type semantics.
  return JSON.stringify(type, (key, value) => {
    if (["typeScriptType", "documentation", "deprecated", "examples"].includes(key)) return undefined;
    // Annotation insertion order is not part of a field's constraints.
    return key === "constraints" && value !== undefined
      ? Object.fromEntries(Object.entries(value).sort(([a], [b]) => constraintOrder.indexOf(a) - constraintOrder.indexOf(b) || (a < b ? -1 : a > b ? 1 : 0)))
      : value;
  });
}
...
function comments(field: SchemaField): string[] {
  return [`TypeScript field: ${field.name}.`, ...field.documentation.split(/\r?\n/).filter(Boolean),
    ...
    ...(field.constraints?.maxLength === undefined ? [] : [`Maximum Unicode code points: ${field.constraints.maxLength}.`]),
    ...
    ...(field.constraints?.itemMinimum === undefined ? [] : [`Minimum array element: ${field.constraints.itemMinimum}.`]),
    ...(field.constraints?.itemMaximum === undefined ? [] : [`Maximum array element: ${field.constraints.itemMaximum}.`]),
    ...(field.constraints?.itemInteger ? ["Every array element must be a safe integer (exactly representable by a JavaScript number)."] : []),
...
export interface LanguageLayout {
  ...
  readonly variants: ReadonlyMap<string, readonly { readonly name: string; readonly type: string; readonly schema: SchemaType; readonly wrapper: string | null }[]>;
}
...
export function renderLanguageTypes(type: SchemaType | ReadonlyMap<string, SchemaType>, language: Language, moduleName: string, namingIdentity: (type: SchemaType) => string = identity): string {
  return compileLanguageTypes(type, language, moduleName, namingIdentity).source;
}

/** Retain the codegen-time layout so validators use exactly the emitted types. */
export function compileLanguageTypes(type: SchemaType | ReadonlyMap<string, SchemaType>, language: Language, moduleName: string, namingIdentity: (type: SchemaType) => string = identity): LanguageLayout {
  const declarations: string[] = [];
  const names = new Map<string, string>();
  const owners = new Map<string, string>();
  const typeNames = new Map<string, string>();
  const unionVariants = new Map<string, { name: string; type: string; schema: SchemaType; wrapper: string | null }[]>();
  ...
  function compile(type: SchemaType, hint: string, root = false): string {
    const name = emit(type, hint, root);
    typeNames.set(identity(type), name);
    return name;
  }
