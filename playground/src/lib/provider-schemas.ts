...
function literalValues(type: SchemaType): Scalar[] | undefined {
  ...
  const values = alternatives.flatMap((alternative) =>
    alternative.kind === "boolean"
      ? [false, true]
      : alternative.kind === "literal" && alternative.value !== null
        ? [alternative.value]
        : [undefined],
  );
...
function objectAlternatives(
  alternatives: readonly ObjectType[],
  ...
  for (const name of names) {
    ...
    const values = fields.map((field) =>
      !field ? [] : field.type.kind === "boolean" ? [false, true] : literalValues(field.type),
    );
    if (values.some((value) => value === undefined)) {
      const omitted = alternatives.filter(
        (_alternative, index) => !fields[index] || fields[index]!.optional,
      );
      const present = alternatives.filter((_alternative, index) => fields[index]);
      if (
        !omitted.length ||
        !present.length ||
        (omitted.length === alternatives.length && present.length === alternatives.length)
      )
        continue;
      return {
    ...
    const groups = choices.map((choice) => ({
      choice,
      alternatives: alternatives.filter((_alternative, index) =>
        choice === undefined
    ...
    for (const group of groups) {
      ...
      const previous = variants.find(
        (variant) => JSON.stringify(variant.schema) === JSON.stringify(schema),
      );
      if (previous) {
        ...
        variants.push({
          values: group.choice === undefined ? [] : [group.choice],
          ...(group.choice === undefined ? { omitted: true } : {}),
          schema,
        });
      }
    }
...
function requestBranches(request: SchemaType): {
  staticRequest: ObjectType[];
  streamingRequest: ObjectType[];
} {
  const alternatives = request.kind === "union" ? request.anyOf : [request];
  const objects = alternatives.filter(
    (alternative): alternative is Extract<SchemaType, { kind: "object" }> =>
      alternative.kind === "object",
  );
  const branches = (kind: "string" | "async-iterable") =>
    objects
      .filter(({ fields }) =>
        fields.some(
          ({ name, type }) =>
            name === "text" &&
            (type.kind === "union" ? type.anyOf : [type]).some((type) => type.kind === kind),
        ),
      )
      .map((object): ObjectType => ({
        ...object,
        fields: object.fields.map((field) =>
          field.name === "text" ? { ...field, type: { kind: "string" } } : field,
        ),
      }));
  const staticRequest = branches("string");
  if (!staticRequest.length)
    throw new TypeError("Provider TtsRequest must contain a static string text branch");
