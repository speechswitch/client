...
test("uses model as the first selector and keeps optional model omission", () => {
  ...
  if (provider.request.kind !== "discriminatedUnion") throw new TypeError("Expected discriminated union")
  ...
  expect(fields.map(({ name }) => name)).toStrictEqual(["model", "output", "speed", "text", "voice"])
...
test("shows model-specific options and retains every same-model timestamp variant", () => {
  expect(objectFields(provider.request, { model: "modern" }).map(({ name }) => name)).toStrictEqual(["model", "timestampGranularity", "language", "output", "speed", "text", "voice"])
  expect(objectFields(provider.request, { model: "modern-fast", timestampGranularity: "character" }).map(({ name }) => name)).toStrictEqual(["model", "timestampGranularity", "language", "output", "speed", "text", "timestampText", "voice"])
  expect(objectFields(provider.request, { model: "dialogue" }).map(({ name }) => name)).toStrictEqual(["model", "language", "output", "text", "voice"])
})

test("a model change removes only unsupported fields and preserves compatible nested output", () => {
  expect(changeSchemaField(provider.request, {
...
test("optional selectors can return to omission without retaining dependent fields", () => {
  ...
  }, "timestampGranularity", "")).toStrictEqual({ model: "modern", text: "Hello", voice: "v" })
  expect(changeSchemaField(provider.request, {
    model: "modern", text: "Hello", voice: "v", language: "fr", speed: 1.1,
  }, "model", "")).toStrictEqual({ text: "Hello", voice: "v", speed: 1.1 })
})

test("saved requests are validated against their model without dropping incompatible fields", () => {
  expect(() => materializedRequest(provider, { model: "dialogue", text: "Hello", voice: "v", speed: 1.1 })).toThrow(expect.objectContaining({ name: "TypeError", message: "request: speed is not supported by this variant" }))
  expect(() => materializedRequest(provider, { model: "missing", text: "Hello", voice: "v" })).toThrow(expect.objectContaining({ name: "TypeError", message: "request: Expected a valid model" }))
  expect(() => materializedRequest(provider, { model: "modern", text: "Hello", voice: "v", timestampText: "normalized" })).toThrow(expect.objectContaining({ name: "TypeError", message: "request: timestampText is not supported by this variant" }))
})
...
test("nested output selectors preserve multiple encodings with the same format", () => {
  ...
  expect(objectFields(output, { format: "mp3" }).map(({ name, schema }) => ({ name, schema }))).toStrictEqual([
  ...
  expect(changeSchemaField(output, { format: "mp3", bitRateBps: 128000 }, "sampleRateHz", 22050)).toStrictEqual({ format: "mp3", sampleRateHz: 22050, bitRateBps: 32000 })
  expect(() => materialize(output, { format: "mp3", sampleRateHz: 22050, bitRateBps: 128000 }, false)).toThrow(expect.objectContaining({ name: "TypeError", message: "request.bitRateBps: Expected one of 32000" }))
  expect(() => materialize(output, { format: "pcm", sampleRateHz: 16000, bitRateBps: 64000 }, false)).toThrow(expect.objectContaining({ name: "TypeError", message: "request: bitRateBps is not supported by this variant" }))
})
...
test("streaming retains model-specific buffering options and supports mixed text unions", async () => {
  ...
  expect(objectFields(streaming, { model: "modern" }).map(({ name }) => name)).toStrictEqual(["model", "textBuffering", "language", "output", "speed", "text", "textBufferThresholds", "voice"])
  expect(objectFields(streaming, { model: "modern", textBuffering: false }).map(({ name }) => name)).toStrictEqual(["model", "textBuffering", "language", "output", "speed", "text", "voice"])
  expect(objectFields(streaming, { model: "dialogue" }).map(({ name }) => name)).toStrictEqual(["model", "language", "output", "text", "voice"])
  ...
  expect(materializedRequest(provider, { model: "dialogue", text: ["Hello", " there"], voice: "v" })).toStrictEqual({ model: "dialogue", text: [{ text: "Hello" }, { text: " there" }], voice: "v" })
})
...
test("both client and server reject invalid streaming combinations", () => {
  ...
  expect(() => providerRequest(request, provider.streamingText)).toThrow(expect.objectContaining(error))
  expect(() => materializedRequest({ ...provider, streamingText: undefined }, request)).toThrow(expect.objectContaining({ name: "TypeError", message: "This provider does not support streaming text" }))
})

test("switching transport reconciles its schema instead of copying one branch's fixed model", () => {
  expect(reconcileValue(provider.streamingText!.request, {
    model: "dialogue", text: "Hello", voice: "v", language: "fr",
  })).toStrictEqual({ model: "dialogue", text: "Hello", voice: "v", language: "fr" })
  expect(reconcileValue(provider.request, {
...
test("presence discriminators expose non-literal fields and their dependent options", () => {
  ...
  expect(fields.map(({ name }) => name)).toStrictEqual(["timestampGranularity", "model", "text", "voice"])
  expect(fields[0]).toStrictEqual({
  ...
  expect(objectFields(presence.request, selected).map(({ name }) => name)).toStrictEqual(["timestampGranularity", "model", "text", "timestampText", "voice"])
  expect(materializedRequest(presence, selected)).toStrictEqual(selected)
  expect(changeSchemaField(presence.request, selected, "timestampGranularity", "[")).toBeUndefined()
  expect(changeSchemaField(presence.request, { ...initial, timestampGranularity: "character", timestampText: "normalized" },
    "timestampGranularity", undefined)).toStrictEqual(initial)
})
...
test("a structured union accepts JSON editor input and validates each original alternative", () => {
  ...
  expect(materializedRequest(presence, request)).toStrictEqual({ ...request, timestampGranularity: ["character"] })
  expect(() => materializedRequest(presence, { ...request, timestampGranularity: ["word"] })).toThrow(expect.objectContaining({
    name: "TypeError",
