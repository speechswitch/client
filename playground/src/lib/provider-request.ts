...
export function streamingTextSegments(values: readonly unknown[]): StreamingTextSegment[] {
  return values.map((value, index) => {
    ...
    if (typeof text !== "string")
      throw new TypeError(`Streaming text segment ${index + 1} must contain text`);
    if (
      delayMs !== undefined &&
      ...
      throw new TypeError(
        `Streaming text segment ${index + 1} delayMs must be a non-negative integer`,
      );
    }
...
export function initialValue(schema: TypeSchema, optional = false): JsonValue | undefined {
  ...
  switch (schema.kind) {
    ...
    case "discriminatedUnion":
      return schema.variants[0] ? initialValue(schema.variants[0].schema) : undefined;
    case "union":
      return schema.variants[0] ? initialValue(schema.variants[0]) : undefined;
    case "json":
      return "";
  }
}
...
export function selectedVariant(schema: DiscriminatedUnionSchema, value: JsonValue | undefined) {
  ...
  if (discriminator === undefined || discriminator === "")
    return schema.variants.find((variant) => variant.omitted);
  if (
...
export function objectFields(schema: TypeSchema, value: JsonValue | undefined): PropertySchema[] {
  ...
  const present = schema.variants.find((variant) => variant.present);
  const original =
    fields.find(({ name }) => name === schema.discriminator) ??
    (present
      ? objectFields(present.schema, value).find(({ name }) => name === schema.discriminator)
      : undefined);
  return [
    {
      ...original,
      ...
      schema: present
        ? original!.schema
        : { kind: "enum", values: schema.variants.flatMap(({ values }) => values) },
    },
    ...fields.filter(({ name }) => name !== schema.discriminator),
  ];
}

export function changeSchemaField(
  schema: TypeSchema,
  ...
  if (schema.discriminator === name) {
    ...
    return selectDiscriminatedVariant(
      schema,
      value,
      next === "" && !schema.variants.some((variant) => variant.present) ? undefined : next,
    );
  }
  const variant = selectedVariant(schema, value) ?? schema.variants[0]!;
    ...
    switch (schema.kind) {
      ...
      case "object": {
        ...
        for (const [name, value] of Object.entries(source)) {
          if (
            value !== undefined &&
            value !== "" &&
            !schema.properties.some((property) => property.name === name)
          ) {
            throw new TypeError(`${name} is not supported by this variant`);
          }
        }
        return Object.fromEntries(
          schema.properties.flatMap((property) => {
            const result = materialize(
              property.schema,
              source[property.name] === undefined ? property.default : source[property.name],
              property.optional,
              `${path}.${property.name}`,
            );
      ...
      case "union": {
        ...
        for (const variant of schema.variants) {
          try {
            return materialize(variant, value, optional, path);
          } catch (cause) {
...
export function selectDiscriminatedVariant(
  ...
  const variant = schema.variants.find((candidate) =>
    discriminator === undefined
      ...
      : candidate.present ||
        ((typeof discriminator === "string" ||
          typeof discriminator === "number" ||
          typeof discriminator === "boolean") &&
          candidate.values.includes(discriminator)),
  );
  if (!variant) throw new TypeError(`Expected a valid ${schema.discriminator}`);
...
export function reconcileValue(
    ...
    return Object.fromEntries(
      schema.properties.flatMap((property) => {
        ...
        try {
          ...
          next =
            property.schema.kind === "object" || property.schema.kind === "discriminatedUnion"
  ...
  try {
    return materialize(schema, value, false);
  } catch {
...
export function materializedRequest(provider: ProviderSchema, request: JsonValue): JsonValue {
  ...
  const streamingSegments = Array.isArray(source?.text)
    ...
    : undefined;
  if (streamingSegments && !provider.streamingText)
    throw new TypeError("This provider does not support streaming text");
  ...
  if (!streamingSegments || !value || typeof value !== "object" || Array.isArray(value))
    return value;
