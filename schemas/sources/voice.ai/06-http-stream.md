...
````yaml /docs/openapi.json post /api/v1/tts/speech/stream
openapi: 3.1.0
info:
...
tags:
  ...
  - name: Managed Tools
    description: >-
      ...
      connection state.
paths:
  /api/v1/tts/speech/stream:
    post:
      tags:
        - Text To Speech
      summary: Speech Stream
      description: >-
        Generate speech from text with HTTP chunked streaming. If voice_id is
        provided, uses that voice; otherwise uses the default built-in voice.
        Returns audio chunks as they are generated.
      operationId: generate_speech_stream_api_v1_tts_speech_stream_post
      requestBody:
        content:
          application/json:
            schema:
      ...
      responses:
        '200':
          description: >-
            Successful Response - Returns HTTP chunked streaming audio (32kHz
            sample rate)
          content:
            audio/mpeg:
              schema:
                ...
                format: binary
                description: >-
                  MP3 audio chunks (32kHz, compressed) streamed via HTTP chunked
                  transfer encoding
            audio/wav:
              schema:
                type: string
                format: binary
                description: >-
                  WAV audio chunks (32kHz, uncompressed) streamed via HTTP
            ...
            audio/pcm:
              schema:
                ...
                format: binary
                description: >-
                  PCM audio chunks (32kHz mono, 16-bit signed little-endian)
                  streamed via HTTP chunked transfer encoding
        '401':
