...
AudioConfig = TypedDict("AudioConfig", {
    "audioEncoding": NotRequired[ReadOnly[Literal["AUDIO_ENCODING_UNSPECIFIED", "LINEAR16", "MP3", "OGG_OPUS", "MULAW", "ALAW", "PCM", "M4A"]]],
...
def is_synthesize_speech_response(value: object) -> TypeGuard[SynthesizeSpeechResponse]:
    return (is_mapping(value) and ("audioContent" not in value or isinstance(value["audioContent"], str)))
...
async def synthesize_speech(value: SynthesizeSpeechRequest, *, base_url: str, headers: Mapping[str, str], transport: HttpTransport) -> HttpResponse:
    ...
    target = urlunsplit((url.scheme, url.netloc, url.path.removesuffix("/") + "/v1/text:synthesize", urlencode(query), ""))
...
async def list_voices(value: ListVoicesInput, *, base_url: str, headers: Mapping[str, str], transport: HttpTransport) -> HttpResponse:
    ...
    target = urlunsplit((url.scheme, url.netloc, url.path.removesuffix("/") + "/v1/voices", urlencode(query), ""))
