...
def is_synthesize_speech_request(value: object) -> TypeGuard[SynthesizeSpeechRequest]:
    return (is_mapping(value) and ("advancedVoiceOptions" not in value or is_advanced_voice_options(value["advancedVoiceOptions"])) and ("audioConfig" not in value or is_audio_config(value["audioConfig"])) and ("enableTimePointing" not in value or (is_sequence(value["enableTimePointing"]) and all((isinstance(value["enableTimePointing"][index1], str) and value["enableTimePointing"][index1] in ("TIMEPOINT_TYPE_UNSPECIFIED", "SSML_MARK",)) for index1 in range(len(value["enableTimePointing"]))))) and ("input" not in value or is_synthesis_input(value["input"])) and ("voice" not in value or is_voice_selection_params(value["voice"])))
...
def _encode_synthesize_speech_request(value: SynthesizeSpeechRequest) -> object:
    ...
    if "enableTimePointing" in value:
        result["enableTimePointing"] = [value["enableTimePointing"][index0] for index0 in range(len(value["enableTimePointing"]))]
    if "input" in value:
...
def is_audio_config(value: object) -> TypeGuard[AudioConfig]:
    return (is_mapping(value) and ("audioEncoding" not in value or (isinstance(value["audioEncoding"], str) and value["audioEncoding"] in ("AUDIO_ENCODING_UNSPECIFIED", "LINEAR16", "MP3", "MP3_64_KBPS", "OGG_OPUS", "MULAW", "ALAW", "PCM", "M4A",))) and ("effectsProfileId" not in value or (is_sequence(value["effectsProfileId"]) and all(isinstance(value["effectsProfileId"][index1], str) for index1 in range(len(value["effectsProfileId"]))))) and ("pitch" not in value or is_number(value["pitch"])) and ("sampleRateHertz" not in value or (isinstance(value["sampleRateHertz"], int) and not isinstance(value["sampleRateHertz"], bool))) and ("speakingRate" not in value or is_number(value["speakingRate"])) and ("volumeGainDb" not in value or is_number(value["volumeGainDb"])))
...
def is_synthesize_speech_response(value: object) -> TypeGuard[SynthesizeSpeechResponse]:
    return (is_mapping(value) and ("audioConfig" not in value or is_audio_config(value["audioConfig"])) and ("audioContent" not in value or isinstance(value["audioContent"], str)) and ("timepoints" not in value or (is_sequence(value["timepoints"]) and all(is_timepoint(value["timepoints"][index1]) for index1 in range(len(value["timepoints"]))))))
