// Package rime implements Rime TTS directly; its upstream wire contracts are partial.
package rime
...
type Options struct {
	...
	// Already authenticated/configured, exclusive override; closed even if the stream is unread.
	WebSocket             runtime.WebSocketLike
	BaseURL, WebSocketURL string
	TimeoutMs             runtime.Optional[int64]
	// Zero selects 4 MiB per socket message.
	MaxMessageBytes int
}
...
func Synthesize(ctx context.Context, request schema.TtsRequest, options Options) (runtime.Input[out.SynthesisItem], error) {
	...
	key, ok := os.LookupEnv("SPEECHSWITCH_RIME_API_KEY")
	if !ok {
		key = os.Getenv("RIME_API_KEY")
	}
	if options.Auth.Rime.Present && options.Auth.Rime.Value.ApiKey.Present {
		key = options.Auth.Rime.Value.ApiKey.Value
	}
	if key == "" {
		return nil, errors.New("Missing auth.rime.apiKey configuration")
	}
	for _, c := range key {
		if c < 32 || c > 126 {
			return nil, errors.New("Rime API key must be a printable ASCII header value")
		}
	}
	socketMode := values.input != nil || values.timed || values.explicitSegmentation || options.WebSocket != nil || options.WebSocketURL != ""
	if options.TimeoutMs.Present && (options.TimeoutMs.Value < 0 || options.TimeoutMs.Value > 2147483647) {
		return nil, errors.New("Rime TimeoutMs must be between 0 and 2147483647")
	}
	...
	if limit < 0 {
		return nil, errors.New("Rime MaxMessageBytes must be positive")
	}
	base := options.BaseURL
	if base == "" {
		base = "https://users.rime.ai"
	}
	httpURL, err := endpoint(base, "/v1/rime-tts", false)
	if err != nil {
		return nil, err
	}
	socketURL := options.WebSocketURL
	if socketURL == "" {
		socketURL = "wss://users-ws.rime.ai/ws3"
	}
	...
	for name, value := range values.wire {
		switch v := value.(type) {
		case string:
			query.Set(name, v)
		case bool:
			query.Set(name, strconv.FormatBool(v))
		case float64:
			query.Set(name, strconv.FormatFloat(v, 'f', -1, 64))
		default:
			return nil, errors.New("Unsupported Rime connection parameter")
		}
	}
	format := values.format
	if format == "ogg_opus" {
		format = "ogg"
	} else if format == "webm_opus" {
		format = "webm"
	}
	query.Set("audioFormat", format)
	query.Set("segment", values.segment)
	...
	s.config.connectSocket = func(ctx context.Context) (runtime.WebSocketLike, error) {
		return runtime.ConnectWebSocket(ctx, socketURL, runtime.WebSocketOptions{Header: http.Header{"Authorization": {"Bearer " + key}}, Transport: options.Transport, MaxMessageBytes: limit})
	}
...
func endpoint(base, path string, socket bool) (string, error) {
	...
	if err != nil || u.Hostname() == "" || u.User != nil || u.Fragment != "" || u.Opaque != "" || strings.ContainsAny(base, "\\\r\n\t ") || (socket && u.Scheme != "ws" && u.Scheme != "wss") || (!socket && u.Scheme != "http" && u.Scheme != "https") {
		return "", errors.New("Invalid Rime endpoint URL")
	}
	if path != "" {
