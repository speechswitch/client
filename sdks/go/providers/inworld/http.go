...
type httpStream struct {
	...
	timed, synchronized, first, terminal, ended bool
	limit                                       int
	pending, chunk                              []byte
	mutex                                       sync.Mutex
}
...
func (s *httpStream) next(ctx context.Context) (out.SynthesisItem, error) {
	for {
		...
		if end > s.limit-len(s.pending) {
			return nil, errors.New("Inworld JSON line exceeds MaxJSONBytes")
		}
		...
		if !utf8.Valid(line) {
			return nil, errors.New("Inworld returned invalid UTF-8")
		}
		if len(bytes.TrimSpace(line)) == 0 {
			continue
		}
		value, err := result(line)
		if err != nil {
			return nil, err
		}
		item, err := audioOutput(value, s.timed, s.synchronized, runtime.Optional[string]{}, nil)
		if err != nil {
			return nil, err
		}
		if item != nil {
...
func (s *singleStream) Next(ctx context.Context) (out.SynthesisItem, error) {
	s.mutex.Lock()
	defer s.mutex.Unlock()
	item := s.item
	s.item = nil
