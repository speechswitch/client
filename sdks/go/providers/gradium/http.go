...
type httpStream struct {
	...
	timed, first, terminal, ended bool
	limit                         int
	pending, chunk                []byte
	mutex                         sync.Mutex
}
...
func (s *httpStream) next(ctx context.Context) (out.SynthesisItem, error) {
	for {
		...
		if end > s.limit-len(s.pending) {
			return nil, errors.New("Gradium JSON line exceeds MaxJSONBytes")
		}
		...
		if !utf8.Valid(line) {
			return nil, errors.New("Gradium returned invalid UTF-8")
		}
		if len(bytes.TrimSpace(line)) == 0 {
			continue
		}
		kind, item, err := packet(line, true)
		if err != nil {
			return nil, err
		}
