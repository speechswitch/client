...
func TestInvalidRequestsFailBeforeIO(t *testing.T) {
	...
	requests = append(requests, schema.TtsRequestAsTurns35afe3cc{Value: schema.TtsRequestTurns35afe3cc{Model: flashSpeakers, Language: "en-US", Speakers: speakers, Turns: []Turn{}, Output: pcm}})
	r := testRequest()
...
func TestProtocolConstraints(t *testing.T) {
	for _, test := range []struct {
		...
		{"unknown alias", "Google dialogue references an unknown speaker: Eve", schema.TtsRequestAsTurns35afe3cc{Value: schema.TtsRequestTurns35afe3cc{Model: flashSpeakers, Language: "en-US", Speakers: speakers, Turns: []Turn{{Speaker: "Eve", Text: "hi"}}, Output: pcm}}},
