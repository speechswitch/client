...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AudioConfigAudioEncoding { AudioEncodingUnspecified, Linear16, Mp3, OggOpus, Mulaw, Alaw, Pcm, M4A, }
impl AudioConfigAudioEncoding { fn as_str(&self) -> &'static str { match self { Self::AudioEncodingUnspecified => "AUDIO_ENCODING_UNSPECIFIED", Self::Linear16 => "LINEAR16", Self::Mp3 => "MP3", Self::OggOpus => "OGG_OPUS", Self::Mulaw => "MULAW", Self::Alaw => "ALAW", Self::Pcm => "PCM", Self::M4A => "M4A", } } }
...
match value.string()?.as_str() { "AUDIO_ENCODING_UNSPECIFIED" => Ok(AudioConfigAudioEncoding::AudioEncodingUnspecified), "LINEAR16" => Ok(AudioConfigAudioEncoding::Linear16), "MP3" => Ok(AudioConfigAudioEncoding::Mp3), "OGG_OPUS" => Ok(AudioConfigAudioEncoding::OggOpus), "MULAW" => Ok(AudioConfigAudioEncoding::Mulaw), "ALAW" => Ok(AudioConfigAudioEncoding::Alaw), "PCM" => Ok(AudioConfigAudioEncoding::Pcm), "M4A" => Ok(AudioConfigAudioEncoding::M4A), _ => Err(Error) }
}
...
let url = endpoint::append(options.base_url, "/v1/text:synthesize").ok_or_else(|| std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid Google HTTP endpoint URL"))?;
...
let mut url = endpoint::append(options.base_url, "/v1/voices").ok_or_else(|| std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid Google HTTP endpoint URL"))?;
if let Some(item) = &value.language_code {
