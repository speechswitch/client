...
export type SynthesizeSpeechRequest = {
  ...
  readonly "enableTimePointing"?: readonly ("TIMEPOINT_TYPE_UNSPECIFIED" | "SSML_MARK")[];
...
export type SynthesizeSpeechResponse = {
  ...
  readonly "timepoints"?: readonly (Timepoint)[];
};
...
export type AudioConfig = {
  readonly "audioEncoding"?: "AUDIO_ENCODING_UNSPECIFIED" | "LINEAR16" | "MP3" | "MP3_64_KBPS" | "OGG_OPUS" | "MULAW" | "ALAW" | "PCM" | "M4A";
...
export function synthesizeSpeech(input: SynthesizeSpeechRequest, options: ClientOptions): Promise<Response> {
  ...
  url.pathname = (url.pathname.endsWith("/") ? url.pathname.slice(0, -1) : url.pathname) + "/v1beta1/text:synthesize";
...
export function listVoices(input: { readonly "languageCode"?: string; }, options: ClientOptions): Promise<Response> {
  const url = new URL(options.baseUrl);
  url.pathname = (url.pathname.endsWith("/") ? url.pathname.slice(0, -1) : url.pathname) + "/v1beta1/voices";
