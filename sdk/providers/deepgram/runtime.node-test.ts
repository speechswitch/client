...
test(
  "Deepgram native Node socket authenticates with headers and completes multiple utterances",
  { timeout: 5000 },
  async () => {
    ...
    const server = await serve((request, socket) => {
      ...
      assert.equal(
        request.url,
        "/proxy/v1/speak?model=aura-asteria-en&encoding=linear16&sample_rate=24000&mip_opt_out=true&speed=1.1",
      );
...
test(
  "Deepgram native socket rejects authentication before advancing input",
  { timeout: 5000 },
  async () => {
    let consumed = false;
    const server = await serve((request, socket) => {
    ...
    try {
      await assert.rejects(
        ...
        { name: "TypeError", message: "WebSocket failed to open" },
      );
...
test(
  "Deepgram native socket closes on abort while the input iterator is stalled",
  { timeout: 5000 },
  async () => {
    let reading!: () => void;
