...
Audio remains `Uint8Array` throughout. REST supports raw PCM/G.711, WAV with PCM/G.711, MP3, Ogg Opus, FLAC, and AAC.
`output.codec` selects the encoding; `output.container` selects raw, WAV, or Ogg
where supported. MP3/FLAC/AAC use provider-fixed framing and omit `container`.
PCM uses `sampleFormat: "int16"`; companded codecs have no PCM sample format. Fixed sample rates are not sent as configurable
...
subsequent text is held until that acknowledgement. `synthesize` yields only raw audio bytes, matching the shared SDK operation.
Control acknowledgements govern the session internally.
...
Schema `@serializeAs` annotations generate the direct speed, model-improvement,
and REST usage-tag mappings consumed by the adapter. Model/voice/language
composition, codec/container conversion, conditional sample rates, and text
framing remain explicit protocol logic.
