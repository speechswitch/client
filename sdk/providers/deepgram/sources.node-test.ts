...
test("the unchanged AsyncAPI repeats overlapping control-message alternatives", async () => {
  const document = YAML.parse(
    await readFile(path.join(root, "schemas/sources/deepgram/01-asyncapi.yaml"), "utf8"),
  );
  assert.deepEqual(
    [
...
test("the OpenAPI models synchronous audio as an empty JSON object", async () => {
  const document = YAML.parse(
    await readFile(path.join(root, "schemas/sources/deepgram/00-openapi.yaml"), "utf8"),
  );
  assert.deepEqual(document.paths["/v1/speak"].post.responses["200"].content, {
    "application/json": {
      schema: { $ref: "#/components/schemas/speak_v1_audio_generate_Response_200" },
    },
  });
...
test("authored model, language, and voice variants cover the catalog in both transports", async () => {
  ...
  const request = extractRepositorySpeechSpec(root).tts.providers.find(
