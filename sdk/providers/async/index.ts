...
} from "../../../schemas/providers/async/index.ts";
import type { ProviderOptions } from "../../options.ts";
import { toSettings } from "../../generated/serializers/async.ts";
import { decodeBase64 } from "../../base64.ts";
import { validateRequest, validateInputItem } from "../../generated/validators/async.ts";
...
interface WireSettings {
  ...
  readonly output_format:
    ...
    | {
        ...
        readonly encoding: "pcm_s16le" | "pcm_f32le" | "pcm_mulaw";
      };
  readonly language?: string;
  readonly speed_control?: number;
...
function settings(request: TtsRequest): WireSettings {
  ...
  const mapped = toSettings(request);
  return {
    ...mapped,
    model_id: (
      {
        "castleflow-1.0": "async_flash_v1.0",
        "flash_v1.5": "async_flash_v1.5",
        "pro_v1.0": "async_pro_v1.0",
      } as const
    )[request.model],
    ...
    output_format:
      output.codec === "mp3"
        ? {
            ...
            container: "mp3",
            bit_rate: mapped.output_format.bit_rate ?? 192000,
          }
        : {
            container: output.container === "wav" ? "wav" : "raw",
            ...mapped.output_format,
            encoding:
              output.codec === "mulaw"
                ? "pcm_mulaw"
...
function decodeMessage(data: unknown): ServerMessage {
  ...
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new TypeError("Async returned an invalid WebSocket message");
    ...
    for (;;) {
      ...
      if (event.kind === "input") {
        if (event.value.done) {
          ...
          validateInputItem(request, event.value.value);
          if (event.value.value.length) {
            contextStarted = true;
            connection.send({
              context_id: contextId,
              transcript: `${event.value.value.replace(/\s+$/u, "")} `,
              force,
            });
          }
        ...
        if ("error_code" in message)
          throw new TypeError(`Async synthesis failed (${message.error_code}): ${message.message}`);
        if (message.context_id !== contextId)
          throw new TypeError("Async returned output for an unexpected context");
        if (message.final && !inputDone)
          throw new TypeError("Async finalized the context before input completed");
...
async function* audio(
  ...
  for await (const chunk of body) {
    ...
    for (let index = 0; index <= bytes.length - quotaMarker.length; index++) {
      if (
        bytes[index] === quotaMarker[0] &&
        quotaMarker.every((byte, offset) => bytes[index + offset] === byte)
      ) {
        marker = index;
        break;
      }
    ...
    while (
      ...
      !quotaMarker
        .subarray(0, retained)
        .every((byte, index) => byte === bytes[bytes.length - retained + index])
    )
      retained--;
...
function timestamped(value: unknown): SynthesisEnvelope<Timestamp<"word">> {
  if (!value || typeof value !== "object")
    throw new TypeError("Async returned an invalid timestamp response");
...
async function responseText(response: Response, config: Configuration): Promise<string> {
  ...
  let text = "";
  for await (const bytes of responseBytes(response.body, config))
...
async function* http(
  ...
  const path =
    request.timestampGranularity === "word"
      ? "/text_to_speech/with_timestamps"
      : request.output.container === "wav"
        ? "/text_to_speech"
  ...
  if (!response.ok)
    throw new TypeError(
      `Async returned HTTP ${response.status}: ${(await responseText(response, config)).trim()}`,
    );
  if (request.timestampGranularity === "word") {
    ...
    yield* audio(
      responseBytes(response.body, config),
...
async function* synthesis(
  ...
  const apiKey =
    options.auth?.async?.apiKey ??
    environment.SPEECHSWITCH_ASYNC_API_KEY ??
    environment.ASYNC_API_KEY;
  ...
  try {
    if (typeof request.text === "string") {
      yield* http(request, request.text, wire, {
        apiKey,
        fetch: options.fetch ?? globalThis.fetch,
        baseUrl: options.baseUrl ?? "https://api.async.com",
        signal,
        aborted,
      });
    } else {
      const url = new URL(
        options.webSocketUrl ?? "wss://api.async.com/text_to_speech/websocket/ws",
      );
      ...
      yield* incremental(
        ...
        wire,
        socket,
        request.segmentation === "immediate",
        signal,
      );
    }
  } finally {
...
/** Streams raw audio bytes for complete or incremental text. */
export async function* synthesize(
  request: TtsRequest,
  options: ProviderOptions = {},
): AsyncIterableIterator<Uint8Array> {
  for await (const value of synthesis(request, options)) {
    if (value instanceof Uint8Array) yield value;
    else if (value.audio) yield value.audio;
  }
}

/** Preserves native word alignment in chunk-correlated audio envelopes. */
export async function* synthesizeWithTimestamps(
  ...
  for await (const value of synthesis(request, options)) {
    if (value instanceof Uint8Array)
      throw new TypeError("Async returned audio without requested timestamps");
    yield value;
  }
}
