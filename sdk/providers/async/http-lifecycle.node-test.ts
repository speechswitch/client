...
test(
  "Async abort interrupts stalled headers and reclaims a late response",
  { timeout: 2000 },
  async () => {
    const controller = new AbortController();
    const reason = new Error("stop headers");
    let deliver!: (response: Response) => void;
    const response = new Promise<Response>((resolve) => {
      deliver = resolve;
    });
    const pending = synthesize(request, {
      auth,
...
for (const mode of ["stream", "wav", "timestamps", "error"] as const)
  test(
    `Async cancels stalled ${mode} body reads independently of fetch`,
    { timeout: 2000 },
    async () => {
      const controller = new AbortController();
      ...
      const input =
        mode === "timestamps"
          ? { ...request, timestampGranularity: "word" as const }
          : mode === "wav"
            ? {
                ...request,
                output: { container: "wav", codec: "pcm", sampleRateHz: 24000 } as const,
              }
            : request;
      const pending = synthesize(input, {
        auth,
        signal: controller.signal,
        fetch: async () => new Response(body, { status: mode === "error" ? 403 : 200 }),
      }).next();
      let timeout: ReturnType<typeof setTimeout> | undefined;
      try {
        await reading;
...
test(
  "Async consumer return aborts its request lifetime without waiting for body cancellation",
  { timeout: 2000 },
  async () => {
...
test("Async preserves proxy path and query and explicitly rejects redirects", async () => {
  ...
  await Array.fromAsync(
    synthesize(request, {
      auth,
      baseUrl: "https://proxy.invalid/root/?tenant=one",
