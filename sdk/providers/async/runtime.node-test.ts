...
test(
  "Node native fetch rejects redirects for all Async HTTP routes",
  { timeout: 5000 },
  async () => {
    const paths: string[] = [];
      ...
      for (const input of [
        ...
        await assert.rejects(
          Array.fromAsync(
            synthesize(input, {
              auth,
              baseUrl: `http://127.0.0.1:${address.port}/proxy?tenant=one`,
            }),
          ),
...
test("Node native fetch streams bytes before the server finishes", { timeout: 5000 }, async () => {
  ...
  const stream = synthesize(
    { ...request, text: "hello" },
    { auth, baseUrl: `http://127.0.0.1:${address.port}` },
  );
  try {
...
test(
  "Node native WebSocket uses the documented query handshake and context protocol",
  { timeout: 5000 },
  async () => {
        ...
        while (pending.length >= 2) {
          ...
          if (message.close_context) {
            const audio = Buffer.from(
              JSON.stringify({ context_id: message.context_id, audio: "AQID", final: true }),
            );
    ...
    try {
      ...
      const chunks = await Array.fromAsync(
        synthesize(
          ...
          {
            auth,
            webSocketUrl: `ws://127.0.0.1:${address.port}/text_to_speech/websocket/ws`,
          },
        ),
      );
      ...
      assert.deepEqual(
        messages.slice(1).map((message) => message.transcript),
        ["hello ", "world ", ""],
      );
    } finally {
