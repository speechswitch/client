...
test.each(["pcm", "wav", "mp3", "mulaw"] as const)("Async schema requires integer %s sample rates", format => {
  ...
  assert.throws(() => validateRequest({ ...request, output: { format, sampleRateHz: 24000.5 } }), TypeError);
});
test("Async schema requires integer MP3 bit rates", () => {
  ...
  assert.throws(() => validateRequest({ ...request, output: { format: "mp3", sampleRateHz: 24000, bitRateBps: 64000.5 } }), TypeError);
});
