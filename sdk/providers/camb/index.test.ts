...
test("generated CAMB integer constraints reject before network and source acquisition", async () => {
  ...
  for (const sampleRateHz of [24000.5, 9007199254740992]) {
    const request = { ...base, text: "Hello", output: { format: "mp3", sampleRateHz } } as const;
    await expect(synthesize(request, {
      auth, fetch: async () => { throw new Error("Unexpected HTTP request"); },
    }).next()).rejects.toEqual(validationError(request));
  }
  let acquired = false;
  const text: AsyncIterable<string> = { [Symbol.asyncIterator]() { acquired = true; return (async function* () { yield "Hello"; })(); } };
  const request = { ...base, text, inferenceSteps: 1.5 };
  await expect(synthesize(request, { auth, webSocket: socket }).next()).rejects.toEqual(validationError(request));
