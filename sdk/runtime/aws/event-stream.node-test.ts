...
test("encodes Polly's signed event-stream frame", () => {
  ...
  expect(Buffer.from(frame).toString("hex")).toBe("000000d1000000433e6299aa053a6461746508000001a03eaf4a20103a6368756e6b2d7369676e6174757265060020" +
...
test("ends a signed event stream with an empty frame", async () => {
  ...
  expect(messages[0]?.body).toStrictEqual(payload);
  expect(messages[1]?.body).toStrictEqual(new Uint8Array());
