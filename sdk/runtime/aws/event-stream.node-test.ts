...
test("encodes Polly's signed event-stream frame", () => {
  ...
  assert.equal(request.signature, "dc4b029653d4113f3b7c270c368cfec4b5efd659d424c071c8f12ecec33db49c");
  ...
  assert.equal(signature, "f42e169dcb500ecd41cf9e93900b24e9f3e6e8f203a981470e4de3c9989123a2");
  assert.equal(Buffer.from(frame).toString("hex"), "000000d1000000433e6299aa053a6461746508000001a03eaf4a20103a6368756e6b2d7369676e6174757265060020" +
...
test("ends a signed event stream with an empty frame", async () => {
  ...
  assert.deepEqual(messages[0]?.body, payload);
  assert.deepEqual(messages[1]?.body, new Uint8Array());
  assert.ok(messages[1]?.headers[":chunk-signature"] instanceof Uint8Array);
});
